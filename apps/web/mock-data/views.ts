...
export interface ViewFilter {
   ...
   unassigned?: boolean;
   /** Only issues assigned to the viewer. Resolved against `currentUserId` at filter time. */
   assigneeId?: 'me';
   filters?: any[];
   [key: string]: any;
}
...
/** Apply an issue view's declarative filter to the issue list. `currentUserId` resolves `assigneeId: 'me'`. */
export function filterIssuesForView(view: View, source: Issue[], currentUserId?: string): Issue[] {
   const { filter } = view;
