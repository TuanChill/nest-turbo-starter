...
| ------------------------------ | ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `lib/workflow-status.tsx`      | `Status`, `StatusCategory`                                                                      | Canonical workflow statuses with SVG icon components and a `category` (`triage` \| `backlog` \| `unstarted` \| `started` \| `completed` \| `canceled`). Also exports `workflowOrderedStatus`, `displayOrderedStatus`, `getStatusesByCategory()`, `StatusIcon`, and reusable icon builders. |
| `mock-data/issues.ts`          | `Issue`                                                                                         | Generated from a compact `seeds` array (**291 unique issues**). `cycleId` links to a cycle ('' = no cycle). `rank` uses LexoRank. `creatorId` is the real author id (from the API, not derived). Helpers: `groupIssuesByStatus`, `sortIssuesByPriority`, `filterIssuesByCycle`, `filterIssuesByCategories`.                                                                                                                                |
...
| `lib/priority-catalog.tsx`     | `Priority`                                                                                      | 5 canonical levels with SVG icon components                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `mock-data/agent.ts`           | `AgentExample`                                                                                  | Agent page example prompt cards, skills list and `chatTitleFrom(input)`; chat responses come from the API and errors are shown instead of using canned replies.                                                                                                                                                                                                                                                                                      |
| `mock-data/inbox.ts`           | `InboxItem`, `NotificationType`                                                                 | Issue-shaped + notification fields (read, user, content)                                                                                                                                                                                                                                                                                                                                                                                   |
| `lib/sidebar-catalog.ts`      | —                                                                                               | Static navigation metadata for the unused legacy sidebar components; active settings navigation is defined in `components/layout/sidebar/nav-settings.tsx`.                                                                                                                                                                                                                                                                                 |
...
- **My issues** (`components/common/my-issues/my-issues.tsx` + `app/[orgId]/my-issues/`)
  ...
  the real `issue.creatorId`/`assignee` fields; Activity reuses
...
- To add a status: add one entry to `status` in `lib/workflow-status.tsx` (append at the
...
pnpm dev           # dev server (Turbopack) on http://localhost:3001
