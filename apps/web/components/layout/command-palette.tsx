...
/** `label -> YYYY-MM-DD` options for the due-date picker, relative to today. */
function getDueDateOptions(): [string, string][] {
   ...
   return [
      ...
      ['End of this week', toIso(endOfWeek)],
      ['In one week', toIso(inOneWeek)],
   ];
}
...
export function CommandPalette() {
   ...
   const users = liveMembers as unknown as User[];
   ...
   const contextIssue = useMemo<Issue | undefined>(() => {
      ...
      return issues.find((issue) => issue.identifier === match[1] || issue.id === match[1]);
   }, [pathname, issues]);
   ...
   const updateIssuePriority = (issueId: string, priority: { id: string }) => {
      const target = issues.find((i) => i.id === issueId || i.identifier === issueId) || issue;
      if (target)
         updateIssueMutation.mutate({
            identifier: target.identifier || target.id,
   ...
   const updateIssueAssignee = (issueId: string, user: { id: string } | null) => {
      const target = issues.find((i) => i.id === issueId || i.identifier === issueId) || issue;
      if (target)
         updateIssueMutation.mutate({
            identifier: target.identifier || target.id,
            data: { assigneeId: user?.id || undefined },
         });
   };
   const updateIssueProject = (issueId: string, project: { id: string } | null | undefined) => {
      const target = issues.find((i) => i.id === issueId || i.identifier === issueId) || issue;
      if (target)
         updateIssueMutation.mutate({
            identifier: target.identifier || target.id,
            data: { projectId: project?.id || undefined },
         });
   };
   const updateIssue = (issueId: string, data: Record<string, unknown>) => {
      const target = issues.find((i) => i.id === issueId || i.identifier === issueId) || issue;
      if (target) updateIssueMutation.mutate({ identifier: target.identifier || target.id, data });
   };
   const addIssueLabel = (issueId: string, label: { id: string }) => {
      const target = issues.find((i) => i.id === issueId || i.identifier === issueId) || issue;
      if (!target) return;
      const currentLabelIds = (target.labels || []).map((l) => l.id);
      updateIssueMutation.mutate({
         identifier: target.identifier || target.id,
         data: { labelIds: [...currentLabelIds, label.id] },
      });
   };
   const removeIssueLabel = (issueId: string, labelId: string) => {
      const target = issues.find((i) => i.id === issueId || i.identifier === issueId) || issue;
      if (!target) return;
      const currentLabelIds = (target.labels || []).map((l) => l.id).filter((id) => id !== labelId);
      updateIssueMutation.mutate({
         identifier: target.identifier || target.id,
         data: { labelIds: currentLabelIds },
      });
   };
         ...
         >
            ...
            <Command className="[&_[cmdk-group-heading]]:text-muted-foreground [&_[cmdk-group-heading]]:px-2 [&_[cmdk-group-heading]]:font-medium [&_[cmdk-group]]:px-2 [&_[cmdk-item]]:px-2 [&_[cmdk-item]]:py-2.5">
               ...
               <CommandList className="max-h-96">
                  ...
                  {route === 'root' && !issue && query.trim() !== '' && (
                     <CommandGroup heading="Issues">
                        {issues.slice(0, 20).map((iss) => (
                           <CommandItem
                              key={iss.id}
                              value={`${iss.identifier} ${iss.title}`}
                              onSelect={() => go(`/issue/${iss.identifier}`)}
                           >
                              <CircleDot className="text-muted-foreground" />
                  ...
                  {route === 'project' && issue && (
                     <CommandGroup heading="Move to project…">
                        ...
                        {allProjects.map((project) => (
                           ...
                           >
                              {renderProjectIcon(project.icon, 'text-muted-foreground size-4')}
                              {project.name}
