...
import { useReviewsDisplayStore } from '@/store/reviews-display-store';
import { ReviewFilter } from './review-filter';
import { ReviewDisplayOptions } from './review-display-options';
...
export default function Reviews({
   ...
   const forYou = allReviews.filter((r) => r.list === 'for-you');
   const createdByYou = allReviews.filter((r) => r.list === 'created');
   const { statuses } = useReviewsFilterStore();
   const { grouping, ordering, showCompleted } = useReviewsDisplayStore();

   const rawSource = listTab === 'for-you' ? forYou : createdByYou;

   const source = React.useMemo(() => {
      let list = rawSource.slice();
      if (!showCompleted) {
         list = list.filter((r) => r.status !== 'merged');
      }
      ...
      return list;
   }, [rawSource, showCompleted, statuses, ordering]);

   const groups = React.useMemo(() => {
      if (grouping === 'none') {
         return [{ label: 'All reviews', items: source }];
      }
      return (['open', 'merged', 'closed'] as ReviewStatus[])
         .map((status) => ({
            label:
               status === 'merged' && listTab === 'for-you' ? 'Completed' : GROUP_LABELS[status],
            items: source.filter((review) => review.status === status),
         }))
         .filter((group) => group.items.length > 0);
   }, [grouping, source, listTab]);
      ...
      <div className="w-full h-full flex overflow-hidden">
         <div className="w-[420px] max-w-[45%] shrink-0 border-r h-full flex flex-col bg-container">
            ...
            <div className="flex-1 overflow-y-auto">
               {isError ? (
                  <QueryErrorState subject="reviews" error={error} onRetry={() => refetch()} />
               ) : isLoading ? (
                  <div className="p-3 space-y-2">
                     {[1, 2, 3].map((i) => (
                        <div
                           key={i}
                           className="flex items-center gap-3 p-2.5 rounded border border-border/30"
                        >
                  ...
                  <div className="p-8 text-center text-xs text-muted-foreground">
                     No reviews matching active filters
                  </div>
               ) : (
                  groups.map((group) => (
