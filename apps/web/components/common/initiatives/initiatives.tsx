                  ...
                  {active === 'owner' && (
                     <CommandGroup>
                        {members.slice(0, 10).map((member) => (
                           <CommandItem
                              key={member.id}
                              onSelect={() => toggleFilter('owner', member.id)}
                           >
                              ...
                              {member.name}
                              {filters.owner.includes(member.id) && (
                                 <CheckIcon className="ml-auto size-3.5" />
                              )}
                           </CommandItem>
                        ))}
...
function ActiveProjectDots({ initiative }: { initiative: Initiative }) {
   ...
   const started = getInitiativeProjects(initiative, liveProjects).filter(
      ...
      >
         ...
         {displayProperties.priority && (
            <span className="hidden sm:flex w-16 shrink-0 items-center">
               {renderPriorityIcon(initiative.priority?.id, 'size-4 text-muted-foreground')}
            </span>
         )}
...
export default function Initiatives() {
   ...
   const displayed = useMemo(() => {
      ...
      return list;
   }, [tab, filters, ordering, initiatives]);
