...
export function CycleDetailsPanel({ cycle, issues }: CycleDetailsPanelProps) {
   ...
   const { mutate: updateCycle } = useUpdateCycle();

   const handleStatusChange = (newStatus: CycleStatus) => {
      if (newStatus === cycle.status) return;
      updateCycle(
         { id: cycle.id, payload: { status: newStatus } },
         { onSuccess: () => toast.success(`Cycle marked as ${cycleStatusLabel[newStatus]}`) }
      );
   };
         ...
         buildBreakdown(
            ...
            (key) => {
               ...
               return {
                  ...
                  leading: project
                     ? renderProjectIcon(project.icon, 'size-3.5 text-muted-foreground shrink-0')
                     : null,
               ...
               <div className="flex items-center gap-1.5 flex-wrap">
                  <DropdownMenu>
                     <DropdownMenuTrigger asChild>
                        <button className="flex items-center gap-1 text-xs px-2 py-1 rounded-md bg-accent text-muted-foreground hover:text-foreground">
                           {cycleStatusLabel[cycle.status]}
                           <ChevronDown className="size-3" />
                        </button>
                     </DropdownMenuTrigger>
                     <DropdownMenuContent align="start">
                        {(Object.keys(cycleStatusLabel) as CycleStatus[]).map((s) => (
