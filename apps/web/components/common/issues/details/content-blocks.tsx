...
/**
 * Lightweight inline formatting: `code` spans, **bold** runs, and (when a
 * `members` map is given) `@memberId` mentions rendered as a highlighted name.
 */
      ...
      <>
         {parts.map((part, index) => {
            ...
            if (part.startsWith('@') && members) {
               const member = members.get(part.slice(1));
               if (member) {
                  return (
                     <span
                        key={index}
                        className="font-medium text-indigo-500 bg-indigo-500/10 rounded px-1"
                     >
                        @{member.name}
                     </span>
                  );
   ...
   const normalizedBlocks = blocks
      ...
      .map((b: unknown) => {
         ...
         if (typeof b === 'object' && b !== null && !('type' in b) && 'text' in b) {
            return { type: 'paragraph', text: String((b as { text: unknown }).text) };
         }
         if (typeof b === 'object' && b !== null && 'type' in b) return b as ContentBlock;
         return null;
      })
               ...
               case 'paragraph':
                  return (
                     <p key={index} className="my-3 text-foreground/90">
                        <InlineText text={block.text} members={members} />
                     </p>
                  );
               case 'bullet-list':
                  return (
                     <ul key={index} className="my-3 list-disc pl-6 space-y-1.5">
                        {block.items.map((item, itemIndex) => (
                           <li key={itemIndex}>
                              <InlineText text={item} members={members} />
                           </li>
                        ))}
                     </ul>
                  );
                     ...
                     <div key={index} className="my-3 space-y-1.5">
                        {block.items.map((item, itemIndex) => (
                           <div key={itemIndex} className="flex items-start gap-2.5">
                              ...
                              >
                                 <InlineText text={item.text} members={members} />
                              </span>
                           </div>
                        ))}
                     </div>
